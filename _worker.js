...
async function 生成配置信息(userID, hostName, sub, UA, RproxyIP, _url, env) {
	if (sub) {
		...
		addresses = addresses.concat('www.speedtest.net:443#TUNEL-Domain-Ipv6');
		if (hostName.includes(".workers.dev")) {
			addressesnotls = addressesnotls.concat(cfips.map(cidr => generateRandomIPFromCIDR(cidr) + '#TUNEL-Domain-Ipv4'));
		} else {
			addresses = addresses.concat(cfips.map(cidr => generateRandomIPFromCIDR(cidr) + '#TUNEL-Domain-Ipv4'));
		}
	}
